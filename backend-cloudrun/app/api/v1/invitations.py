...
async def delete_invitation_code(
    ...
    3. 删除关联企业的设备告警 (Alarm)
    4. 删除关联企业的日分析报告 (DailyReport)
    5. 删除关联企业的用户账号 (User)
    6. 删除关联企业的设备 (Device)
    7. 删除企业本身 (Organization)
    8. 删除邀请码 (InvitationCode)
    ...
    try:
        ...
        if org:
            ...
            if device_ids:
                alarms_result = await db.execute(
                    delete(Alarm).where(Alarm.device_id.in_(device_ids))
                )
                deleted_alarms = alarms_result.rowcount or 0

                daily_reports_result = await db.execute(
                    delete(DailyReport).where(DailyReport.device_id.in_(device_ids))
                )
                deleted_daily_reports = daily_reports_result.rowcount or 0
        ...
        if org:
            logger.info(
                ...
                deleted_daily_reports=deleted_daily_reports,
