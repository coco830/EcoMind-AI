...
from app.services.regulatory_service import RegulatoryService, RateLimitError
...
async def get_regulator_overview(
    ...
    try:
        ...
        logger.exception("Regulator overview failed", error=str(exc))
...
async def get_regulator_heatmap(
    ...
    try:
        ...
        logger.exception("Regulator heatmap failed", error=str(exc))
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(exc))
...
async def get_regulator_trends(
    ...
    try:
        ...
        logger.exception("Regulator trends failed", error=str(exc))
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(exc))
...
async def get_regulator_consistency(
    ...
    try:
        ...
        logger.exception("Regulator consistency failed", error=str(exc))
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(exc))
...
async def get_regulator_brief(
    ...
    service = RegulatoryService(db)
    try:
        return await service.get_ai_brief(
            current_user,
            period,
            target_date,
            year,
        ...
        detail = "今日简报生成次数已达上限，请明日再试" if exc.period == "daily" else "本月简报生成次数已达上限，请下月再试"
        raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail=detail)
    except ValueError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="invalid period")
    except Exception as exc:
        logger.exception("Regulator brief failed", error=str(exc))
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(exc))


@router.get("/brief/download")
async def download_regulator_brief(
    current_user: Annotated[User, Depends(require_regulator_access)],
    db: Annotated[AsyncSession, Depends(get_db)],
    period: str = Query("daily", pattern="^(daily|monthly)$"),
    target_date: date | None = Query(None),
    ...
    service = RegulatoryService(db)
    try:
        payload = await service.get_ai_brief(
            current_user,
            period,
            target_date,
            year,
    ...
    if format == "excel":
        file_content = await service.generate_brief_excel(payload)
        media_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        extension = "xlsx"
    else:
        file_content = await service.generate_brief_pdf(payload)
        media_type = "application/pdf"
        extension = "pdf"

    filename = f"regulator_brief_{payload['period_label']}.{extension}"
    encoded_filename = quote(filename, safe="")
