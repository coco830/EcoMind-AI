...
from app.models.organization import Organization, OrganizationCreate, OrganizationResponse, OrganizationStatus, OrganizationType
...
class JurisdictionOption(BaseModel):
    """Available jurisdiction code option for invitations."""
...
async def list_organizations(
    ...
    only_invited: bool = Query(True, description="Only organizations bound to invitation codes"),
    ...
    if only_invited:
        query = query.join(InvitationCode, InvitationCode.org_id == Organization.id).distinct()
...
@router.get("/scopes", response_model=list[JurisdictionOption])
async def list_jurisdiction_scopes(
    db: Annotated[AsyncSession, Depends(get_db)],
    current_user: Annotated[User, Depends(require_superadmin)],
    level: str = Query(..., description="Scope level: district or park"),
) -> list[JurisdictionOption]:
    """List available jurisdiction codes from existing enterprise organizations."""
    if level not in {"district", "park"}:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid scope level",
        )

    if level == "park":
        code_col = Organization.park_code
        name_col = Organization.park_name
    else:
        code_col = Organization.region_code
        name_col = Organization.region_name
