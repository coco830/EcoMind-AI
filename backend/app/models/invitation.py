...
class InvitationCode(Base):
    ...
    description: Mapped[str | None] = mapped_column(Text)  # 备注说明

    # 关联的组织 - 创建邀请码时自动创建对应组织
    org_id: Mapped[UUID | None] = mapped_column(GUID, ForeignKey("organizations.id"))
    ...
    expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))  # 过期时间
    ...
    # 创建者
    created_by: Mapped[UUID | None] = mapped_column(GUID)  # 创建此邀请码的管理员
...
class InvitationCodeCreate(BaseSchema):
    ...
    expires_days: int | None = Field(None, ge=1, description="有效天数，不填则永不过期")
...
class InvitationCodeResponse(BaseSchema):
    ...
    org_id: UUID | None = None  # 关联的组织ID
    org_name: str | None = None  # 组织名称（方便前端展示）
    max_uses: int
    used_count: int
