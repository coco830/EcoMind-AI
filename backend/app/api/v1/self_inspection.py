...
from app.core.masking import is_demo_viewer, mask_device_name, mask_org_name
...
async def get_device_flow(
    ...
    if mask and org:
        org_name = mask_org_name(org_id=str(org.id), org_code=org.code)
    else:
    ...
    for device in devices:
        ...
        device_flow_list.append(
            DeviceFlowResponse(
                device_id=device.mn,
                device_name=(mask_device_name(device_id=str(device.id), mn=device.mn) if mask else device.name),
...
async def get_device_online_metrics(
    ...
    for d in devices:
        device_name = mask_device_name(device_id=str(d.id), mn=d.mn) if mask else d.name
